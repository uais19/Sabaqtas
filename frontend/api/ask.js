...
// Второе условие: насколько тема выделяется на фоне остального учебника.
//
// Зачем оно нужно. Одного порога не хватает: вопрос «Как решить систему
// линейных уравнений?» набирает 0.664, хотя систем уравнений в наших
// учебниках нет, а честный вопрос «Как сложить 1/4 и 1/6?» — только 0.647.
// Причина в том, что вектор ловит «математичность» текста, а не тему:
// любой школьный вопрос с формулами похож на любой параграф с формулами.
//
// Отрыв так не обмануть. Если вопрос действительно по теме, несколько
// кусков одного параграфа выделяются резко, а остальной учебник остаётся
// далеко внизу. Если вопрос «вообще про математику», он одинаково средне
// похож на всё сразу, и выделяться нечему.
//
// Число получено замером на 38 вопросах (tools/check_threshold.py):
// у своих вопросов отрыв 0.124 и выше, у чужих 0.084 и ниже.
// ВАЖНО: после загрузки новых учебников замер надо повторить —
// с ростом корпуса фон меняется, а вместе с ним и это число.
const FOCUS_THRESHOLD = 0.104;
...
// Среднее по куску массива чисел.
...
// Насколько тема выделяется на фоне остального учебника.
//
// Берём средний балл трёх лучших кусков и вычитаем средний балл нижней
// половины. Три — потому что одна тема почти всегда разбита на несколько
// кусков подряд (у §6 алгебры за 8 класс их семь), и брать только лучший
// значит недооценивать правильный вопрос. Нижняя половина — это фон:
// куски, к вопросу заведомо не относящиеся.
//
// Половина, а не «всё после десятого места», нарочно: доля не сломается,
// когда учебников станет больше.
//
// scoresSorted — баллы всех кусков, уже отсортированные по убыванию.
function focusScore(scoresSorted) {
  const top = scoresSorted.slice(0, 3);
  const background = scoresSorted.slice(Math.floor(scoresSorted.length / 2));
  return average(top) - average(background);
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Насколько тема выделяется на фоне остального учебника.
    // scored уже отсортирован по убыванию — берём из него только баллы.
    const focus = focusScore(scored.map(function (item) {
      return item.score;
    }));

    // Шаг 3: два условия отсечения. Первое — лучший кусок слишком не похож
    // на вопрос. Второе — вопрос похож на весь учебник одинаково, то есть
    // ни на какую конкретную тему. Не выполнено любое — в учебниках ответа
    // нет, и модель не вызываем вовсе. Это гарантия в коде, а не просьба
    // к модели: инструкцию можно уговорить нарушить, условие — нельзя.
    if (topScore < SIMILARITY_THRESHOLD || focus < FOCUS_THRESHOLD) {
