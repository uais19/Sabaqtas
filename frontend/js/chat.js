...
// --- Задача из адреса страницы ---

// Экран заданий передаёт задачу в параметре ask. Подставляем её в поле
// ввода, но НЕ отправляем: ученик должен видеть, с чем идёт к наставнику,
// и может дописать своё. Одно нажатие «Отправить» — и разговор начат.
//
// Раньше кнопка «Помоги решить самому» открывала пустой чат: она обещала
// помощь с этой задачей, а чат спрашивал, с какой именно.
//
// Курсор ставим в конец строки, чтобы дописывать было удобно сразу.
function applyAskFromUrl() {
  const params = new URLSearchParams(window.location.search);
  ...
  input.value = ask;
  input.focus();
  input.setSelectionRange(ask.length, ask.length);
}

applyAskFromUrl();

// --- Язык ответа из выбора интерфейса ---

// Ученик уже выбрал язык на входе — переспрашивать в чате незачем.
// Ставим его как начальный; переключатель в чате остаётся рабочим,
// потому что язык интерфейса и язык объяснения — не всегда одно и то же:
// ученик может читать экраны по-казахски, а разбор просить по-русски.
//
// uiLang живёт в i18n.js. Если он почему-то не загрузился, молча остаёмся
// на русском: работающий чат важнее, чем угаданный язык.
if (typeof uiLang === "function") {
  setLang(uiLang());
}
